...
export function parseFare(fareString) {
  // Your code here
  let parsedString = parseFloat(fareString);
  if (Number.isNaN(parsedString) || typeof fareString !== "string") return -1;
  return parsedString;
}
...
export function findCheapestAndCostliest(...fares) {
  ...
  let filteredFares = fares.filter((x) => typeof x === "number");
  if (filteredFares.length == 0) return null;

  return {
    cheapest: Math.min(...filteredFares),
    costliest: Math.max(...filteredFares),
  };
}
