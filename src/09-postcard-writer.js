...
export function isValidPincode(code) {
  ...
  if (code.startsWith("0") || code.length < 6 || code.length > 6) return false;
  return /^\d+$/.test(code);
}
...
export function isFromState(address, stateCode) {
  // Your code here
  if (typeof address !== "string" || address === null) return false;
  if (typeof stateCode !== "string" || stateCode === null) return false;
...
export function countVowels(message) {
  ...
  if (typeof message !== "string" || message === null || message === "")
    return 0;
  let result = message.match(/[aeiouAEIOU]/gi);
